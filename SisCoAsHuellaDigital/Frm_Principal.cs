...
namespace MicroSisPlani
{
    public partial class Frm_Principal : Form
    {
        ...
        {
            Frm_Filtro fil = new Frm_Filtro();
            Form_Print_Fechas asis = new Form_Print_Fechas();
            Form_Solo_Personal_por_Fechas solo = new Form_Solo_Personal_por_Fechas();
            ...
            DateTime xdiaini = solo.dtp_fechaIni.Value;
            DateTime xdiafin = solo.dtp_fechaFin.Value;

            fil.Show();
            asis.CualImprimir = idper;
            asis.FechaInicio = xdiaini;
            asis.FechaFin = xdiafin;
            asis.ShowDialog();
            fil.Hide();
        }
        ...
        {
            Frm_Filtro fil = new Frm_Filtro();
            Form_Contrato per = new Form_Contrato();

            //Obtener el ID personal
            if (lsv_person.SelectedIndices.Count == 0)
            {
                fil.Show();
                MessageBox.Show("Selecciona al Personal para Registrar su Contrato", "Advertencia de Seguridad", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                fil.Hide();
            }
            else
            {
                ...
                fil.Show();
                per.saveeditar = true;
                per.buscarPersonalContratoEditar(xidsocio);
                per.Buscar_Personal_ParaEditar(xidsocio);
                per.ShowDialog();
                fil.Hide();
